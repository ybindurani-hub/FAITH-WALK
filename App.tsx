...
// 100+ Languages sorted alphabetically by name
const LANGUAGES = [
...
const App: React.FC = () => {
  ...
  const NavButton = ({ view, label, icon }: { view: View; label: string; icon: React.ReactNode }) => (
    <button
      ...
      className={`flex flex-col items-center justify-center w-full py-3 transition-all duration-300 ${
        currentView === view ? 'text-indigo-600 -translate-y-1' : 'text-slate-400 hover:text-slate-600'
      }`}
    >
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-slate-50 font-sans overflow-hidden">
      {/* Top Header */}
      <header className="bg-white/90 backdrop-blur-md border-b border-slate-100 px-4 py-3 flex items-center justify-between sticky top-0 z-40 shadow-sm shrink-0">
        <div className="flex items-center gap-2 text-indigo-700">
          ...
          <h1 className="text-lg font-serif font-bold tracking-tight text-slate-800">FaithWalk AI</h1>
        </div>
        ...
        <div className="flex items-center gap-2">
           <div className="relative">
             <select 
               ...
               className="appearance-none bg-slate-50 border border-slate-200 text-slate-700 py-2 pl-3 pr-8 rounded-lg text-xs font-bold uppercase tracking-wide focus:outline-none focus:ring-2 focus:ring-indigo-500 max-w-[140px] shadow-sm hover:border-indigo-300 transition-colors"
             >
               {LANGUAGES.map(lang => (
      ...
      {/* Main Content Area - All views are rendered but hidden based on state */}
      ...
      <nav className="bg-white/90 backdrop-blur-lg border-t border-slate-200 pb-safe z-40 shrink-0">
