      ...
      <div className="p-3 md:p-6 w-full flex-1 flex flex-col">
        {!sermon && (
          <div className="text-center mb-6 pt-4 shrink-0 animate-in fade-in slide-in-from-top-4">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-slate-800 dark:text-indigo-100 mb-2">Pulpit AI</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base">Homiletics & Sermon Preparation</p>
          </div>
        )}

        {!sermon && (
          <div className="bg-white dark:bg-slate-900 p-4 md:p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800 flex flex-col gap-4 md:gap-6 max-w-2xl mx-auto w-full transition-colors">
            ...
            <label className="w-full relative group">
              ...
              <input
                type="text"
                className="w-full p-3 md:p-4 pr-14 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-base md:text-lg font-serif"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <label className="block">
                ...
                <select value={audience} onChange={(e) => setAudience(e.target.value)} className="w-full p-3 md:p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none appearance-none font-medium">
                  {AUDIENCES.map((a) => <option key={a} value={a}>{a}</option>)}
                </select>
              </label>

              <label className={`flex flex-col justify-center p-3 md:p-4 rounded-xl border cursor-pointer transition-all ${includeDeepContext ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-200 dark:border-indigo-700' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-indigo-200'}`}>
            ...
            <button onClick={handleBuild} disabled={loading || !topic} className="w-full mt-2 bg-indigo-600 text-white py-3 md:py-4 rounded-xl font-bold text-lg shadow-lg hover:bg-indigo-700 hover:-translate-y-0.5 disabled:opacity-50 transition-all">
              Generate Sermon
            </button>
          </div>
        )}
        ...
        <div className="flex-1 pb-4">
          {sermon && (
            <div className="bg-white dark:bg-slate-900 p-4 md:p-12 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 animate-in fade-in slide-in-from-bottom-8 relative transition-colors">
              ...
              <div className="prose prose-indigo dark:prose-invert prose-base md:prose-lg max-w-none font-serif text-slate-700 dark:text-slate-300">
